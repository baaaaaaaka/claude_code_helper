...
EOF
  ...
  if [[ ! -x "$CLAUDE_CLI_TEST_BIN" ]]; then
    echo "missing claude CLI test binary at $CLAUDE_CLI_TEST_BIN" >&2
    exit 1
  fi

  echo "[centos7 tui] verifying patched Claude enters TUI via Go integration test"
  env \
    "${run_env[@]}" \
    CLAUDE_PATCH_TEST=1 \
    ...
    "$CLAUDE_CLI_TEST_BIN" -test.run '^TestClaudePatchIntegration(|RetriesKnownFailure)$' -test.count=1 -test.v
