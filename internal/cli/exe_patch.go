...
const (
	...
	// Claude 2.1.133+ flattened policy settings source guards to direct returns.
	policySettingsDirectReturnStage1 = `function\s+[A-Za-z0-9_$]+\s*\([^)]*\)\s*\{\s*if\(\s*(?:[A-Za-z0-9_$]+\s*={2,3}\s*['"]policySettings['"]|['"]policySettings['"]\s*={2,3}\s*[A-Za-z0-9_$]+)\s*\)\s*return\s+[^;{}]+;`
)
...
func bypassPermissionDecisionPatchSpec() (exePatchSpec, error) {
	return exePatchSpec{
		...
		applyID: "bypass-permission-decision-v3",
		apply: func(data []byte, log io.Writer, preview bool) ([]byte, exePatchStats, error) {
